...
export async function storeConversation(
  ...
  const id = `conversation_${Date.now()}_${Math.random()
    .toString(36)
    .substring(7)}`;
...
export async function updateConversationFeedback(
  ...
  try {
    ...
    // Update metadata only (using update instead of upsert)
    await index.update({
      id: conversationId,
      metadata: {
        ...(conv.metadata || {}),
    ...
    console.log(
      `[Learning] Updated feedback for ${conversationId}: ${feedback}`
    );

    // If negative feedback, mark for improvement
    if (feedback === "negative" && conv.metadata) {
      await markForImprovement(
        conversationId,
...
export function extractTopics(question: string): string[] {
  ...
  if (
    lowerQuestion.match(
      /\b(javascript|typescript|react|next\.?js|node\.?js|python|java)\b/i
    )
  ) {
  ...
  if (
    lowerQuestion.match(
      /\b(project|built|created|developed|github|repository)\b/i
    )
  ) {
  ...
  if (
    lowerQuestion.match(
      /\b(education|degree|university|college|study|learn)\b/i
    )
  ) {
...
export async function reinforcePositivePatterns(conversationId: string) {
  try {
    ...
    const question = conv.metadata.question as string;
    const answer = conv.metadata.answer as string;
    
    if (!question || !answer) {
      console.warn("[Learning] Missing question or answer in metadata");
      return;
    }
  ...
  try {
    ...
    return results.map((r) => ({
      ...
      suggestedImprovement:
        "Review and update the knowledge base with more accurate information",
    }));
  } catch (error) {
