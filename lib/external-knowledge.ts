 ...
 */
export function getCurrentDateTime(query: string): ExternalKnowledgeResponse | null {
  const timeKeywords = ['time', 'date', 'today', 'now', 'current', 'what day'];
  const isTimeQuery = timeKeywords.some(keyword => query.toLowerCase().includes(keyword));
...
export function calculateMath(query: string): ExternalKnowledgeResponse | null {
  const mathKeywords = ['calculate', 'what is', 'how much', '+', '-', '*', '/', 'multiply', 'divide', 'add', 'subtract'];
  const hasMathKeyword = mathKeywords.some(keyword => query.toLowerCase().includes(keyword));
...
export function getDefinition(query: string): ExternalKnowledgeResponse | null {
  const defKeywords = ['what is', 'what does', 'define', 'meaning of', 'full form', 'abbreviation'];
  const hasDefKeyword = defKeywords.some(keyword => query.toLowerCase().includes(keyword));
  ...
  const definitions: Record<string, string> = {
    ...
    'kubernetes': 'Kubernetes (K8s) is a container orchestration platform for automating deployment.',
    ...
    'agile': 'Agile is an iterative approach to software development emphasizing flexibility.',
    'scrum': 'Scrum is an Agile framework with sprints, stand-ups, and iterative development.',
    ...
    'microservice': 'Microservices are small, independent services that work together.',
    'monolith': 'A monolithic application is a single, unified software architecture.',
    'frontend': 'Frontend is the client-side part of an application that users interact with.',
    'backend': 'Backend is the server-side part that handles logic, databases, and APIs.',
    'fullstack': 'Full-stack refers to both frontend and backend development.',
    'framework': 'A framework is a pre-built structure for developing applications.',
    'library': 'A library is a collection of reusable code functions.',
    'repository': 'A repository (repo) is a storage location for code and version history.',
    'commit': 'A commit is a saved snapshot of changes in version control.',
    'branch': 'A branch is an independent line of development in version control.',
    ...
    'refactoring': 'Refactoring is improving code structure without changing behavior.',
    'deployment': 'Deployment is releasing software to a production environment.',
    'production': 'Production is the live environment where users access the application.',
    'staging': 'Staging is a pre-production environment for testing.',
    'localhost': 'Localhost (127.0.0.1) refers to your own computer in networking.',
    ...
    'middleware': 'Middleware is software that connects different applications or services.',
    ...
    'token': 'A token is a piece of data used for authentication or authorization.',
    'encryption': 'Encryption converts data into code to prevent unauthorized access.',
    'hashing': 'Hashing converts data into a fixed-size string (one-way function).',
    ...
    'async': 'Async (asynchronous) means operations that don\'t block code execution.',
    'promise': 'A Promise represents a future value from an asynchronous operation.',
    'callback': 'A callback is a function passed to another function to be executed later.',
    'event': 'An event is an action that occurs in the system (click, load, etc.).',
    ...
    'props': 'Props are arguments passed to components (like function parameters).',
    ...
    'virtual dom': 'Virtual DOM is a lightweight copy of the DOM for efficient updates.',
    ...
    'mobile first': 'Mobile-first design starts with mobile layout then scales up.',
    'breakpoint': 'A breakpoint defines when a layout changes for different screen sizes.',
    ...
    'babel': 'Babel is a JavaScript compiler that converts modern JS to older versions.',
    'webpack': 'Webpack is a module bundler for JavaScript applications.',
    'vite': 'Vite is a fast build tool and development server.',
    'eslint': 'ESLint is a tool for identifying and fixing JavaScript code issues.',
    ...
    'integration test': 'Integration tests verify how different parts work together.',
    ...
    'tech debt': 'Technical debt is the cost of quick solutions that need refactoring later.',
    ...
    'api key': 'An API key is a unique identifier for authenticating API requests.',
    'environment variable': 'Environment variables store configuration outside code.',
 ...
 */
export function getLocationInfo(query: string): ExternalKnowledgeResponse | null {
  const locationKeywords = ['where are you', 'your location', 'where do you live', 'where from', 'location'];
  const isLocationQuery = locationKeywords.some(keyword => query.toLowerCase().includes(keyword));
  
  if (!isLocationQuery) return null;

  return {
    answer: "I'm based in Nepal, and I work as a Full-Stack Developer. I'm available for remote opportunities globally!",
 ...
 */
export function getWeatherInfo(query: string): ExternalKnowledgeResponse | null {
  const weatherKeywords = ['weather', 'temperature', 'forecast', 'rain', 'sunny', 'cold', 'hot'];
  const isWeatherQuery = weatherKeywords.some(keyword => query.toLowerCase().includes(keyword));
  
  if (!isWeatherQuery) return null;

  return {
    answer: "I don't have access to real-time weather data yet, but you can check weather.com or your local weather service for current conditions!",
 ...
 */
export function getExternalKnowledge(query: string): ExternalKnowledgeResponse | null {
