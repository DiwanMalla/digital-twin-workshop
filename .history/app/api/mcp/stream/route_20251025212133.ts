  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        // Enhance query for better project matching
        const enhancedQuery = question.toLowerCase().includes('project') 
          ? `${question} portfolio projects built developed` 
          : question;
        
        const results = await index.query({
          data: enhancedQuery,
          topK: 15, // Increased to get more results including all projects
...
5. Use ONLY information from above - never make up or assume anything
6. Be specific with numbers, technologies, and achievements from the data
7. If something isn't in the data, say "I don't have that specific information in my profile"
