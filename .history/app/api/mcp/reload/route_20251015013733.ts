...
function createChunksFromJson(
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    if (typeof value === "object" && value !== null && !Array.isArray(value)) {
      ...
      chunks.push(...createChunksFromJson(value as Record<string, unknown>, currentPrefix, key));
      ...
      value.forEach((item, i) => {
        ...
        if (typeof item === "object" && item !== null) {
          ...
          const title = `${key} - ${
            (item as Record<string, unknown>).name || (item as Record<string, unknown>).title || `Item ${i + 1}`
          }`;
          chunks.push({
            id: itemId,
            title,
          ...
          chunks.push(...createChunksFromJson(item as Record<string, unknown>, itemId, key));
        } else {
          // Simple list item
          chunks.push({
            id: itemId,
...
export async function DELETE() {
  ...
  try {
    ...
    console.log(
      `[Reload] Processing ${allChunks.length} total chunks (${
        allChunks.length - contentChunks.length
      } from JSON + ${contentChunks.length} content_chunks)...`
    );
    ...
    console.log(
      `[Reload] Successfully reloaded ${vectors.length} vectors in ${duration}ms`
    );

    return NextResponse.json({
      success: true,
      message: `Database reloaded with ${vectors.length} total profile data chunks`,
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          error instanceof Error ? error.message : "Failed to reload database",
      },
      { status: 500 }
    );
  }
