...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent, voiceText?: string) => {
    e.preventDefault();
    const textToSubmit = voiceText || question.trim();
    if (!textToSubmit || isLoading || isStreaming) return;
  ...
  const handleFeedback = async (
    ...
    if (!message || message.type !== "ai" || !message.conversationId) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log(`[Feedback] Submitted ${feedback} for conversation`);
      }
    } catch (err) {
      ...
      <div className="relative min-h-screen flex">
        ...
        >
          ...
          <div className="mb-4">
            <a
              href="/job-matcher"
              className="block p-3 sm:p-4 rounded-xl border border-blue-500/30 backdrop-blur-sm bg-blue-500/5 hover:bg-blue-500/10 transition-all group"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
              ...
              {messages.length === 0 && !isStreaming && (
                <div className="flex flex-col items-center justify-center min-h-[50vh] sm:min-h-[60vh] text-center px-4">
                  ...
                  <p className="text-slate-400 text-xs sm:text-base mb-6 max-w-2xl">
                    I&apos;m an AI assistant trained on Diwan&apos;s profile.
                ...
                <div className="flex gap-2 sm:gap-4 justify-start animate-fade-in">
                  ...
                  <div className="max-w-[85%] sm:max-w-[80%] lg:max-w-[75%] rounded-2xl px-3 py-2.5 sm:px-5 sm:py-4 bg-white/5 backdrop-blur-sm text-slate-100 border border-blue-500/30 shadow-lg shadow-blue-500/20">
                    ...
                    <div className="flex items-center gap-2 mt-2 pt-2 border-t border-white/10">
                      ...
                      <p className="text-xs text-blue-400">Streaming...</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-3">
                ...
                {messages.length === 0 && !isStreaming && (
                  <div className="flex flex-wrap gap-1.5 sm:gap-2">
                    <span className="text-xs text-slate-500 py-1.5">Try:</span>
